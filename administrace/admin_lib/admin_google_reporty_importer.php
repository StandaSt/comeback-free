<?php
// administrace/admin_lib/admin_google_reporty_importer.php * Verze: V13 * Aktualizace: 09.09.2026
...
$dbConnectPath = $projectRoot . '/www/common/db/db_connect.php';
...
const REPORT_IMPORT_LOG_FILE = __DIR__ . '/../../provoz/log/reporty_import.txt';
...
function renderGoogleDataPreview(): void
{
    ...
    $actionUrl = function_exists('cb_url') ? (string)cb_url('/provoz.php') : '/provoz.php';
    ?>
...
function main(): array
{
        ...
        foreach (findWorkbooksForBranch($branchPath, $branchName) as $workbook) {
            ...
            foreach ($sheetTargets as $sheetName => $sheetPath) {
                ...
                foreach ($reports as $report) {
                    ...
                    $zpracovaneDny[$datum][$branchName] = true;
                    try {
    ...
    ksort($zpracovaneDny);
    $zpracovaneDnyVystup = [];
    foreach ($zpracovaneDny as $datum => $pobocky) {
        $zpracovaneDnyVystup[] = [
            'date' => $datum,
            'branches' => array_keys($pobocky),
        ];
    }

    logLine('zpracovane dny: ' . count($zpracovaneDnyVystup));
    ...
    return ['days' => $zpracovaneDnyVystup];
}
...
{
    echo $text . "<br>\n";
    if (defined('CB_GOOGLE_DATA_LIBRARY') && CB_GOOGLE_DATA_LIBRARY === true) {
        return;
    }
    @ob_flush();
    flush();
