...
try {
    $cbObdobiSummaryOd = new DateTimeImmutable((string)$cbObdobiOd);
    $cbObdobiSummaryDo = new DateTimeImmutable((string)$cbObdobiDo);
    $cbObdobiSummary = $cbObdobiSummaryOd->format('j.n.Y H:i') . ' - ' . $cbObdobiSummaryDo->format('j.n.Y H:i');
} catch (Throwable $e) {
    $cbObdobiSummary = 'Období není zvoleno';
}
?>
