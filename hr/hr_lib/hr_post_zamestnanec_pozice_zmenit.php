...
function hr_post_zamestnanec_pozice_zmenit(mysqli $db, int $idUser): void
{
    ...
    try {
        if (!cb_pravo_ma(307)) {
            throw new CbUserVisibleException('Nemáte právo měnit pozici zaměstnance.');
        }
        $slot = trim((string)($_POST['id_slot'] ?? ''));
        if ($idPerson <= 0 || preg_match('/^\d+$/', $slot) !== 1) {
            throw new CbUserVisibleException('Vyberte zaměstnance a pozici.');
        }
        ...
        cb_form_finish(hr_pracovni_pomer_url($idPerson), false, cb_hr_chyba_text($e, 'Změna pozice zaměstnance', ['table' => 'hr_zarazeni']));
    }
}
