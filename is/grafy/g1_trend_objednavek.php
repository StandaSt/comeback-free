<?php
// G1: trend objednavek podle mesice
...
foreach ($branchOrder as $idPob) {
    $dataByPob[$idPob] = array_fill(0, count($trendMonthKeys), 0);
}
...
if ($stmt instanceof mysqli_result) {
    while ($row = $stmt->fetch_assoc()) {
        ...
        if (isset($trendMonthIndex[$mesic], $dataByPob[$idPob])) {
            $dataByPob[$idPob][(int)$trendMonthIndex[$mesic]] += $cnt;
        }
    }
    $stmt->free();
}
...
$json = $jsonEncode($payload, 'Nepodařilo se připravit data pro trendový graf.');

return $renderGrafTile('G1', 'Trend objednávek podle měsíce', $periodLabel, 'graf_max_1', $json, '', 'g1_trend_objednavek');
