...
$modulePath = $cbNormalizePath((string)($GLOBALS['CURRENT_MODULE'] ?? ''));
if ($modulePath !== '/' && ($appPath === $modulePath || str_starts_with($appPath, $modulePath . '/'))) {
    $appPath = substr($appPath, strlen($modulePath));
    $appPath = ($appPath === false || $appPath === '') ? '/' : $appPath;
    $appPath = $cbNormalizePath((string)$appPath);
}
