<?php
// karty/obj_zakaznici.php * Verze: V7 * Aktualizace: 07.03.2026
...
/*
 * Cache COUNT(*) na kratky cas v session:
 * - stejny filtr netaha pocet zaznamu opakovane pri strankovani
 * - fallback je normalni SQL dotaz, kdyz session neni aktivni
 */
function fetchTotalCached(mysqli $conn, string $sql, array $params, string $types, string $cacheKey, int $ttlSec = 30): int {
    $now = time();
    $sessionReady = (session_status() === PHP_SESSION_ACTIVE);

    if ($sessionReady) {
        if (!isset($_SESSION['cb_count_cache']) || !is_array($_SESSION['cb_count_cache'])) {
            $_SESSION['cb_count_cache'] = [];
        }

        $bucket = $_SESSION['cb_count_cache'][$cacheKey] ?? null;
        if (is_array($bucket)) {
            $ts = (int)($bucket['ts'] ?? 0);
            $val = (int)($bucket['val'] ?? -1);
            if ($val >= 0 && ($now - $ts) <= $ttlSec) {
                return $val;
            }
        }
    }

    $stmtCnt = stmtExec($conn, $sql, $params, $types);
    ...
    if ($sessionReady) {
        $_SESSION['cb_count_cache'][$cacheKey] = [
            'ts'  => $now,
            'val' => $total,
        ];
    }
...
try {
    ...
    /* ===== COUNT (s kratkou cache) ===== */
    $countSql = 'SELECT COUNT(*) FROM ' . $fromSql . $whereSql;
    $countKey = 'obj_zakaznici:' . md5($countSql . '|' . $types . '|' . json_encode($params));
    $total = fetchTotalCached($conn, $countSql, $params, $types, $countKey, 30);
