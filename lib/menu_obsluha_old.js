// lib/menu_obsluha.js * Verze: V4 * Aktualizace: 13.2.2026
'use strict';
 ...
 * - Přepnutí menu režimu (?menu=...) zatím zůstává plný reload (mění se struktura menu)
 */
...
(function (w) {
  ...
  /* =========================================================
     0) AJAX navigace (jen obsah <main>)
     ========================================================= */
  ...
  function fetchMainAndSwap(url, push) {
    ...
    fetch(url, {
      ...
      // Vložíme obsah stránky do <main> (typicky .page-head + .card)
      ...
      if (push) {
        try {
          ...
          // když pushState selže, nevadí (obsah už je zobrazen)
        }
      }
    }).catch((err) => {
      ...
      // fallback: když AJAX selže, uděláme klasický reload
      w.location.href = url;
    });
  }
  ...
  function buildUrlWithParam(paramName, paramValue) {
    ...
    try {
      ...
      // Při přechodu na jinou stránku vždy vynuluj stránkování (p),
      // aby se nepřenášela stránka z jiné sekce.
      if (name === 'page') {
      ...
      if (name === 'page') {
        return base + '?page=' + encodeURIComponent(value);
      }
      if (name === 'menu') {
        return base + '?menu=' + encodeURIComponent(value);
      }
  ...
  function setParamAndGo(paramName, paramValue) {
    ...
    // AJAX jen pro ?page=... (mění se jen obsah v <main>)
    if (name === 'page') {
      wirePopstateOnce();
      fetchMainAndSwap(url, true);
      return;
    }

    // Přepínač menu režimu (dropdown/sidebar) zatím řešíme reloadem
    w.location.href = url;
  }
  ...
  function renderDropdown(rootEl, opts) {
    ...
    MENU.forEach((sec) => {
      ...
      // ===== jednotné chování (timer + otevření/zavírání) =====
...
// lib/menu_obsluha.js * Verze: V4 * řádků 574 * Aktualizace: 13.2.2026
// konec souboru