...
function cb_ai_analytik_sql_normalizovat(string $sql): string
{
    ...
    if ($sql === '' || str_contains($sql, "\0")) {
        throw new RuntimeException('AI navrhla prázdný nebo neplatný SQL dotaz.');
    }
    ...
    for ($i = 0; $i < $length; $i++) {
        ...
        if ($state === 'normal') {
            if (($char === '-' && $next === '-' && ($i + 2 >= $length || ctype_space($sql[$i + 2])))
                || $char === '#'
                || ($char === '/' && $next === '*')) {
                throw new RuntimeException('SQL komentáře nejsou v AI dotazu povoleny.');
            }
            if ($char === "'") {
                $state = 'single';
                $out .= ' ';
    ...
    if ($state !== 'normal') {
        throw new RuntimeException('AI navrhla SQL s neuzavřeným literálem.');
    }
    ...
    if (str_contains($trimmed, ';')) {
        throw new RuntimeException('AI smí předat pouze jeden SQL dotaz.');
    }
...
function cb_ai_analytik_sql_overit(string $sql): string
{
    ...
    if (preg_match('/^\s*(SELECT\b|WITH\b)/i', $normalized) !== 1) {
        throw new RuntimeException('AI smí používat pouze SELECT dotazy.');
    }
    ...
    if (preg_match($forbidden, $normalized) === 1
        ...
        throw new RuntimeException('AI navrhla SQL operaci, která není v read-only režimu povolena.');
    }
