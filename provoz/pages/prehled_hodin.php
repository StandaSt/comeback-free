...
$psBuildUrl = static function (array $extra = []) use ($psBaseParams, $psQueryDefaults): string {
    return cb_url_query('index.php', array_merge($psBaseParams, $extra), $psQueryDefaults);
};
$psResetUrl = cb_url_query('index.php', ['m' => 'provoz', 'page' => 'prehled_hodin'], $psQueryDefaults);
...
$psExportQuery = http_build_query($psExportParams, '', '&', PHP_QUERY_RFC3986);
$psExportSuffix = $psExportQuery !== '' ? '?' . $psExportQuery : '';
$psExportPdfUrl = cb_root_url('provoz/lib/export_prehled_smen_pdf.php') . $psExportSuffix;
$psExportTxtUrl = cb_root_url('provoz/lib/export_prehled_smen_txt.php') . $psExportSuffix;
$psExportXlsxUrl = cb_root_url('provoz/lib/export_prehled_smen_xlsx.php') . $psExportSuffix;
?>
<div class="provoz_prehled_hodin_page">
  <?php if ($psError !== ''): ?>
  ...
  <?php else: ?>
    <form method="get" action="<?= h(cb_root_url('index.php')) ?>" autocomplete="off" data-cb-max-form="1">
      ...
      <input type="hidden" name="page" value="prehled_hodin">
      ...
      <div class="card-max-pagination list-bottom">
        <div class="per-form gap_8 displ_inline_flex">
          <span>Zobrazuji</span>
          <select name="ps_per" class="filter-input ram_sedy txt_seda bg_bila zaobleni_8 vyska_24 per-select">
