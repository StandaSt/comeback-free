...
if ($scriptName !== '') {
    ...
    if ($dir !== '') {
        ...
        foreach ($parts as $part) {
            if (in_array($part, $CB_MODULES, true)) {
                $CURRENT_MODULE = $part;
                break;
            }
        }
...
if ($CURRENT_MODULE === '') {
    ...
    $hostModule = strtolower((string)($hostParts[0] ?? ''));
    if (in_array($hostModule, $CB_MODULES, true)) {
        $CURRENT_MODULE = $hostModule;
    }
}
...
function cb_url(string $path): string
{
    ...
    return cb_root_url($path);
}

function cb_root_url(string $path = ''): string
{
...
{
    return (string)($GLOBALS['CURRENT_MODULE'] ?? '');
}

function cb_current_module_url(string $path = ''): string
{
    $module = cb_current_module();
    if ($module === '') {
        return cb_url($path);
    ...
    return rtrim(cb_module_url($module), '/') . '/' . ltrim($path, '/');
}

function cb_module_asset_url(string $path, string $module = ''): string
{
    $module = strtolower(trim($module));
    if ($module === '') {
        $module = cb_current_module();
